...
function PalmIcon({ size = 23 }: { size?: number; strokeWidth?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12.1 10.4c-.4 3.8-.1 7.2 1 10" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/><path d="M12 10.5c-1.8-3.8-5.6-4.2-8-2.1 2.5.1 4.2 1 5.2 2.5M12 10.5c1.3-4.1 5.1-5.5 8-4.2-2.2.7-3.8 1.9-4.6 3.7M12 10.5c-.4-4-3.1-6.4-6-6.4 1.7 1.4 2.8 3.1 3.1 5.2M12 10.5c.9-3.8.1-6.5-1.7-8.5 2.8.5 4.5 2.9 4.4 6.1" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/><path d="M9.5 21h7" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>;
}
...
export default function SiteHeader() {
  ...
  const siteSchema = { "@context":"https://schema.org", "@graph":[{ "@type":"Organization", "@id":"https://tripownia.pl/#organization", name:"Tripownia.pl", url:"https://tripownia.pl/", logo:"https://tripownia.pl/tripownia-logo.webp" },{ "@type":"WebSite", "@id":"https://tripownia.pl/#website", url:"https://tripownia.pl/", name:"Tripownia.pl", publisher:{ "@id":"https://tripownia.pl/#organization" }, inLanguage:"pl-PL" }] };
  return <>
    <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(siteSchema).replace(/</g,"\\u003c")}}/>
    <header className="menu-v5"><div className="menu-v5-shell">
      ...
      <nav className="menu-v5-primary" aria-label="Najważniejsze sekcje">
        {primaryItems.map(item=>{ if("seasonal" in item&&item.seasonal&&!showMarkets)return null; const Icon=item.icon; return <Link key={item.label} className={`menu-v5-card menu-v5-${item.tone}`} href={item.href}><span className="menu-v5-icon" aria-hidden="true"><Icon size={23} strokeWidth={2.2}/></span><span className="menu-v5-copy">{"badge" in item&&item.badge?<span className="menu-v5-badge">{item.badge}</span>:null}<strong>{item.label}</strong><small>{item.note}</small></span></Link>})}
