...
export default function Home() {
  ...
  function focusSearch() {
    const section = document.getElementById("wyszukiwarka");
    section?.scrollIntoView({ behavior: "smooth", block: "start" });
    window.setTimeout(() => document.getElementById("tripownia-search-query")?.focus(), 450);
  }
...
<section className="hero"><div className="shell hero-inner">
    ...
    <section className="section shell custom-trip"><div className="section-heading"><div><div className="kicker">WŁASNA PODRÓŻ</div><h2>Masz pomysł? Zbuduj wyjazd po swojemu.</h2><p>Wybierz kierunek, lotnisko, długość i budżet. Tripownia pomoże połączyć lot, nocleg i atrakcje zamiast wciskać gotowy pakiet.</p></div></div><div className="hub-grid custom-trip-grid"><button type="button" className="hub-action-card" onClick={focusSearch}><strong>🧩 Zacznij od własnych parametrów</strong><span>Ustaw kierunek i lotnisko. W wynikach dostaniesz też linki do lotów z tymi parametrami.</span><em>Otwórz wyszukiwarkę →</em></button><a href={partners.esky.buildUrl("https://www.esky.pl/tanie-loty/")} target="_blank" rel="sponsored noopener noreferrer"><strong>✈️ Loty eSky</strong><span>Sprawdź tanie loty w eSky. Po wyborze kierunku w wyszukiwarce Tripownii link dostanie konkretny cel.</span></a><a href={partners.kiwi.buildUrl()} target="_blank" rel="sponsored noopener noreferrer"><strong>🛫 Loty Kiwi.com</strong><span>Alternatywna wyszukiwarka lotów. W wynikach Tripownii ustawiamy kierunek i najniższą cenę.</span></a><a href={partners.booking.buildUrl()} target="_blank" rel="sponsored noopener noreferrer"><strong>🏨 Dobierz nocleg</strong><span>Przejdź do Booking.com z identyfikatorem afiliacyjnym Tripowni.</span></a></div></section>

    <section className="section shell content-hubs polished-hubs"><div className="section-heading"><div><div className="kicker">ODKRYWAJ Z TRIPOWNIĄ</div><h2>Więcej niż dzisiejsza selekcja</h2><p>Nie znalazłeś nic na dziś? Przejdź od razu do konkretnego typu podróży i zacznij od ofert, nie od pustej inspiracji.</p></div></div><div className="hub-grid"><Link href="/?trip=inspiracje&focus=destination#wyszukiwarka"><strong>🌍 Kierunki</strong><span>Wybierz miejsce i od razu przejdź do ofert oraz partnerów.</span><em>Znajdź kierunek →</em></Link><Link href="/?trip=city-break#wyszukiwarka"><strong>🏙 City break</strong><span>Krótkie wyjazdy, lot + nocleg i aktualne okazje.</span><em>Zobacz city breaki →</em></Link><Link href="/?trip=wakacje#wyszukiwarka"><strong>🏖 Wakacje i Last Minute</strong><span>All Inclusive, słońce i wyjazdy z polskich lotnisk.</span><em>Znajdź wakacje →</em></Link><Link href="/poradniki"><strong>🧭 Poradniki</strong><span>Formalności, lotniska, bagaż i praktyczne wskazówki.</span><em>Przejdź do poradników →</em></Link><Link href="/parkingi"><strong>🚗 Parkingi</strong><span>Wybierz lotnisko i przejdź bezpośrednio do rezerwacji parkingu.</span><em>Znajdź parking →</em></Link><Link href="/atrakcje"><strong>🎟 Atrakcje</strong><span>Pomysły na miejscu i zakup biletów u partnerów.</span><em>Zobacz atrakcje →</em></Link></div></section>
    <section className="section shell score-section score-section-polished" id="score"><div className="score-copy"><div className="kicker">WYNIKI TRIPOWNIA.PL</div><h2>Cena to dopiero początek.</h2><p>Oceniamy wyjazd całościowo: cenę, pogodę, hotel, termin i lot. Jeden wynik zamiast pięciu luźnych parametrów.</p><div className="score-benefits"><span>✓ cena względem terminu</span><span>✓ pogoda i sezon</span><span>✓ jakość hotelu</span><span>✓ wygoda lotu</span></div><Link className="score-link" href="/okazje">Zobacz oferty z oceną Tripownii <ArrowRight size={16}/></Link></div><div className="score-box score-box-premium"><div className="score-label">PRZYKŁADOWA OCENA</div><div className="bigscore">9,6<span>/10</span></div>{[['Cena','10/10'],['Pogoda','8/10'],['Hotel','9/10'],['Termin','10/10'],['Lot','9/10']].map(([a,b])=><div className="score-row" key={a}><span>{a}</span><strong>{b}</strong></div>)}<div className="verdict">🔥 BIERZEMY</div></div></section>
    <SiteFooter />
</main>;
}
